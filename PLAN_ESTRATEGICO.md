...
*   **Frontend**: React 19, Vite, TanStack Router, TanStack Query, shadcn/ui.
    *   *Estado*: **Excelente**. El stack es moderno, performante y bien estructurado.
*   **Backend**: Simulación con `json-server`.
    *   *Estado*: **Crítico**. No apto para producción. No soporta lógica de negocio compleja ni seguridad real.
*   **Base de Datos**: Archivo `mocks/db.json`.
    *   *Estado*: **Crítico**. Sin integridad referencial, transacciones ni escalabilidad.
*   **Autenticación**: Integración parcial con Clerk en frontend.
    *   *Estado*: **Inseguro**. El backend simulado no valida los tokens, permitiendo acceso irrestricto a la API si se conoce la URL.
...
*   **UI Adaptativa**:
    *   Ocultar botones de "Eliminar" o "Editar" para usuarios *Viewer*.
...
*   **Backend**: Migración a **TanStack Start (Server Functions)**.
    *   *Beneficio*: Type-safety de extremo a extremo, menor latencia, mismo lenguaje/stack.
*   **Base de Datos**: **PostgreSQL** (Neon/Supabase) + **Drizzle ORM**.
    *   *Beneficio*: Escalabilidad, integridad SQL, migraciones sencillas.
*   **Caching**:
