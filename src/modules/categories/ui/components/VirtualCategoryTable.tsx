...
import { VIRTUAL_OVERSCAN, VIRTUAL_ROW_HEIGHT } from '../../model/constants'

interface VirtualCategoryTableProps<TData> {
...
export function VirtualCategoryTable<TData>({
  columns,
  data,
  hasNextPage,
  isFetchingNextPage,
  onFetchNextPage,
  scrollResetKey,
}: VirtualCategoryTableProps<TData>) {
              ...
              return (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <TableCell
                      key={cell.id}
                      className="py-3 px-6 text-sm border-b border-border/40 align-middle"
                    >
