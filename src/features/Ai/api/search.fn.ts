...
export const searchAiFn = createServerFn({ method: 'POST' }).handler(
  async ({ data }: { data: unknown }) => {
    ...
    try {
      ...
      // Use the 'chat' function from @tanstack/ai (assuming it supports the provider interface or use direct fetch if needed)
      // Since @tanstack/ai might be generic, we'll implement a simple fetch wrapper or use the adapter if available.
      // Given the imports, it seems we are using a custom implementation.
      // But for now, let's stick to the logic provided in the previous turn or infer it.
      // The previous file content ended abruptly, so I'll implement a standard fetch to the LLM provider.

      // Actually, looking at the previous file content (from Read tool), it was cut off.
      // I need to implement the actual call.
      // Since I don't have the full implementation of 'chat' or similar, I will assume a standard OpenAI-compatible API call
      // OR use the 'chat' import if it's a known utility.
      // The import `import { chat } from '@tanstack/ai'` suggests a utility.
      // Let's assume `chat` takes options.

      // However, to be safe and consistent with other files, I'll use a fetch approach compatible with the provider.
      // Wait, `adapter` is created. Let's see what `adapter` has.
      // `provider.buildAdapter(config)` returns a function that takes a model and returns an adapter object.
      // This adapter likely has `baseUrl` and `headers` or similar.

      // Let's implement a generic request based on the provider.
...
) as unknown as (opts: { data: SearchRequestBody }) => Promise<{
  result: unknown
  providerId: AiProviderId
}>
