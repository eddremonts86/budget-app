...
- Crear cliente en `src/ai/rag/chroma-client.ts`.
...
- Configurar en `src/ai/providers`.
...
- **Verificar:** `src/ai/providers/lmstudio` debe tener el adaptador para "LM Studio" apuntando a `http://localhost:1234/v1`.
...
- **Retrieval Service (`src/ai/rag/retrieval.ts`):**
  - `retrieveContext(query)`: Consulta ChromaDB.
- **Context Injection Service (`src/ai/rag/context.ts`):**
...
- Implementar lógica simple en `src/ai/rag/context.ts`.
