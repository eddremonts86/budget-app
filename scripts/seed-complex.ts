...
async function seed() {
  ...
  try {
    ...
    await db.delete(schema.projects) // Projects first if they reference teams? No, projects reference teams via array (no FK constraint usually unless explicitly defined, schema says text array)
    // Actually schema definition: team: text('team').array(). No foreign key constraint on array elements in Drizzle usually.
    // But verify: projectId in todos references projects.id.
    // team leadId references users.id.
    ...
    // Set reporting structure (everyone reports to one of the first 5 managers)
    const managers = usersData.slice(0, 5)
    usersData.forEach((u, i) => {
      if (i >= 5) {
        // @ts-ignore
        u.reportsTo = managers[i % 5].id
      }
    })
    ...
    for (let i = 0; i < 5; i++) {
      departmentsData.push({
        id: `dept_${randomUUID()}`,
        name: deptNames[i],
        managerId: usersData[i].id, // First 5 users are managers
    ...
    for (let i = 0; i < 8; i++) {
      const lead = usersData[gen.randomInt(0, 10)] // Pick a lead from first 10 users
    ...
    const projectStatuses = ['planning', 'active', 'on_hold', 'completed', 'active'] // Weighted active
    ...
    for (let i = 0; i < 10; i++) {
      ...
      projectsData.push({
        ...
        team: assignedTeams, // Using 'team' column which stores IDs array
      })
    }
    ...
    for (let i = 0; i < 1234; i++) {
      ...
      // Pareto Principle for Priority: 20% High, 60% Medium, 20% Low
      const rand = Math.random()
      let priority = 'medium'
      if (rand < 0.2) priority = 'high'
      else if (rand > 0.8) priority = 'low'

      const estimatedTime = gen.randomInt(1, 40) // 1-40 hours
      const actualTime =
        Math.random() > 0.3 ? Math.floor(estimatedTime * (0.8 + Math.random() * 0.4)) : null

      // Status distribution
      const statuses = ['pending', 'in_progress', 'completed', 'pending', 'in_progress'] // Weighted
      let status = gen.randomItem(statuses)
      if (priority === 'high' && Math.random() < 0.1) status = 'blocked' // Edge case
      ...
      todosData.push({
        ...
        status: status as any, // Cast because of enum mismatch if 'blocked' is not in enum (I added it conceptually but need to verify enum)
        // Wait, I updated todoStatusEnum in schema?
        // No, I only updated schema.ts text content.
        // The enum defined in schema.ts is: export const todoStatusEnum = pgEnum('todo_status', ['pending', 'in_progress', 'completed'])
        // I DID NOT UPDATE THE ENUM VALUES in schema.ts code provided in SearchReplace.
        // I only saw the text update for tables.
        // I need to check if I updated the enum.
        ...
        dependencies: i > 10 && Math.random() < 0.1 ? [`todo_${randomUUID()}`] : null, // Fake ID or real? Better to use real IDs but complex in loop.
        // Real IDs: previous ones.
      })

      // Fix dependencies to use real IDs
      if (i > 0 && todosData[i].dependencies) {
        // @ts-ignore
        todosData[i].dependencies = [todosData[gen.randomInt(0, i - 1)].id]
      }
    }

    // Batch insert todos (1234 might be too big for one query depending on driver, but usually fine. Split if needed)
    const chunkSize = 100
